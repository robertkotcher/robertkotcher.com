...
import { Archive, LogOut, Paperclip, RefreshCw } from "lucide-react";
...
function attachmentUrl(attachment: unknown) {
  const url = attachmentValue(attachment, "download_url") || attachmentValue(attachment, "url");
  const content = attachmentValue(attachment, "content");
  if (url) return url;
  if (!content) return null;

  return `data:${attachmentValue(attachment, "content_type") || "application/octet-stream"};base64,${content}`;
}
...
export default function InboxPage() {
  ...
  async function archiveSelectedThread() {
    ...
    setArchiving(true);
    setError("");
    const response = await fetch(`/api/inbox/messages/${encodeURIComponent(selected.thread.id)}/archive`, { method: "POST" });
    const data = await response.json().catch(() => ({}));
    setArchiving(false);
    if (response.status === 401) return setAuthenticated(false);
    if (!response.ok) return setError(data.error || "Could not archive that thread.");

    setThreads((current) => current.filter((thread) => thread.id !== selected.thread.id));
    setSelected(null);
    setReply("");
    setError("Archived.");
  }
